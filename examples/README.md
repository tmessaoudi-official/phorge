...
| `guide/html.phg` | the `core.html` escape **kernel** — `text` (auto-escape), `raw` (audited trust), `render`; `Html` is a distinct type from `string`, XSS-safe by construction (core.html Wave 1) |
...
| `core.html` escape kernel: `text`/`raw`/`render`; `Html` ≠ `string` (XSS-safe by construction) | `guide/html` |
