...
| Method overloading (`foo(int)` / `foo(string)`) | 🚧 M-RT | confirmed for the milestone — lowers to one dispatching PHP method (PHP forbids same-name redeclaration); sequenced right after S5 |
| Class `extends`, traits | 🚧 M-RT | the Rich Types milestone — `instanceof` (S1), interfaces (S2), `Map` (S3), unions (S4), intersections (S5), generics (S7), generic methods, generic classes shipped; these are later slices |
