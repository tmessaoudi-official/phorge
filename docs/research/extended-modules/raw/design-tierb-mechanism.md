...
| **Supervision trees / let-it-crash** | **A** | Downstream of the actor runtime; restart = match on strategy + counter over the deterministic scheduler; Phorge already has byte-identical faults (M-faults Slice 2). |
