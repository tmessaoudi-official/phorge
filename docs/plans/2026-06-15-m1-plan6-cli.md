...
**Goal:** `phg run file.phg` runs a program end-to-end; `check`/`parse`/`lex`
...
```rust
//! Phorj CLI: `phg <run|check|parse|lex> <file>`. Thin dispatcher over the
//! testable `phorj::cli` module.

use std::process::exit;

use phorj::cli;

const USAGE: &str = "usage: phg <run|check|parse|lex> <file>";
...
| **Docs** | Spec records CLI-1..CLI-6. Update `handoff.md`: M1 **complete (6/6)**, CLI usage, PHP-converter as next milestone. New public API `phorj::cli::{cmd_run, cmd_check, cmd_parse, cmd_lex}`. |
| **Config** | `[[bin]] phorj` already in Cargo.toml — no change. State it. |
...
- **Type consistency:** every `cmd_*` is `fn(&str) -> Result<String, String>`; `main`
  dispatches all four; `BIN` uses `CARGO_BIN_EXE_phg`. Error field accesses
