...
`foo.phg` on the VM with no Phorge install — by copying the running phg binary, embedding the
...
git commit -m "feat(cli): phg build — embed source as a .phorge section for host target (M2.5 Phase 1)"
```
