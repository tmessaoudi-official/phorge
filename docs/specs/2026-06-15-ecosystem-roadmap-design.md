# Phorge Ecosystem Strategy & ROI Roadmap — Design

> Strategy for the whole Phorge ecosystem (backends, PHP interop, extensions, stdlib,
> packages, concurrency, server, tooling) and a priority/ROI-ranked milestone+sprint
> roadmap. Builds on the language design (`2026-06-15-phorge-language-design.md`) and the
> M2 VM design (`2026-06-15-m2-bytecode-vm-design.md`). Brainstormed & locked 2026-06-15.

## 1. Strategic reframe — Phorge has two backends; that's the key asset

- **Phorge → PHP transpiler** (M1, done, round-trip-verified): runs anywhere PHP runs;
...
Phorge apps test/deploy via the PHP backend (PHPUnit, Composer, PHP hosting). Native
...
| PHP→Phorge migration tool (typed PHP 8.x subset, **batch/offline**) | ✅ later (M8) | One-way; best-effort with human review; reuses transpiler infra |
| "Rebuild PHP→Phorge on the spot" (live transpile) | ❌ reject | Sound static typing vs dynamic PHP; runtime type failures; undecidable inference over arbitrary PHP |
...
Unlike PHP's loose `register_function`, every native module or imported symbol in Phorge
must register **both**:
...
| E-6 | Testing | One Phorge test surface + `TestRunner` trait; PHPUnit-backed now (bootstrap), native `phorge test` later |
...
| **M7** | Native test runner + breadth + DX | native `phorge test` (E-6) · MySQL/Redis connectors · HTTP client · `phorge fmt` | DX + ecosystem breadth |
| **M8** | PHP→Phorge migration tool (typed-subset, batch) | reverse transpiler infra + type inference over typed PHP 8.x | Onboard existing PHP codebases (clean subset) |
...
- Package manifest format details (`phorge.toml` schema) — at M5.
