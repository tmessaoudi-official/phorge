...
pub(super) fn file_read(args: &[Value], _: &mut String) -> Result<Value, String> {
    match args {
...
pub(super) fn file_exists(args: &[Value], _: &mut String) -> Result<Value, String> {
    match args {
        [Value::Str(path)] => Ok(Value::Bool(std::path::Path::new(path).exists())),
        _ => Err("File.exists expects (string)".into()),
    }
}
pub(super) fn file_write(args: &[Value], _: &mut String) -> Result<Value, String> {
    match args {
