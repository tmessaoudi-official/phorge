...
//! we write that file:
//!   * `PHORGE_BAKE_STUB_MANIFEST=<path>` set (the CI primary build) → copy the file's contents in;
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=PHORGE_BAKE_STUB_MANIFEST");
    ...
    let contents = match std::env::var_os("PHORGE_BAKE_STUB_MANIFEST") {
        Some(path) => {
            // Re-run if the source manifest itself changes.
            println!("cargo:rerun-if-changed={}", path.to_string_lossy());
            std::fs::read_to_string(&path).unwrap_or_else(|e| {
                panic!(
                    "PHORGE_BAKE_STUB_MANIFEST={} could not be read: {e}",
